...
export function useChartFormat(
  ...
  const handleXAxisTickFormat = useCallback(
    (value: string) => {
      if (isDailyTab) return isMobile ? value.slice(0, 2) : isTablet ? value.slice(0, 3) : value
    ...
    [isDailyTab, isMobile, isTablet, isDesk, isSmallDesk, units.timeUnit, currentChartTab],
  )

  const yTicks = useMemo(() => getTicks(activeData), [activeData])
  const yDomain = useMemo(() => [yTicks[0], yTicks[yTicks.length - 1]], [yTicks])
  ...
  return useMemo(
    ...
    [handleXAxisTickFormat, yTicks, yDomain, aspect, isDailyTab, units.temperatureUnit],
  )
}
