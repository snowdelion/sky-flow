...
import { favoriteStore, recentStore, useSearchStore, type HistoryItem } from "@/entities/location"
...
const setup = () => {
  ...
  const setRecentItem = (data: HistoryItem[] = historyData) => act(() => recentStore.update(data))
  ...
  return {
    ...
    getLink: (option: HTMLElement, name: RegExp) => within(option).getByRole("button", { name }),

    getIcon: (option: HTMLElement, name: RegExp) => within(option).getByLabelText(name),
  }
}
...
describe("Search integration", () => {
  ...
  it("should remove city from favorites", async () => {
    ...
    await waitFor(() => {
      ...
      const hasCity = currentFavorites.some((item) => /berlin/i.test(item.city))
      expect(hasCity).toBe(false)
    })
  })
  ...
  describe("focus/blur behavior", () => {
    ...
    it("shouldn't close dropdown when clicking inside", async () => {
      const { user, input, findTab, findOption, getIcon, setRecentItem, setFavoritesItem } = setup()
      setRecentItem()
      setFavoritesItem()
      ...
      const removeFavoriteIcon = getIcon(favoriteOption, /remove from favorites/i)
    ...
    it("should hide kayboard on mobile after clicking inside", async () => {
      ...
      if (!touchStart) Object.defineProperty(window, "ontouchstart", { value: true })
      ...
      if (!touchStart) Object.defineProperty(window, "ontouchstart", { value: false })
    })
  })
})
