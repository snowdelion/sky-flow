...
import { createGeoData, createResultsMocks, server } from "@/shared/lib/testing"
...
describe("fetchSearchResults", () => {
  ...
  it("should throw AppError if API returns null", async () => {
    ...
    await expect(fetchSearchResults({ geoData, units: DEFAULT_UNITS })).rejects.toThrow(
      /validation failed/i,
    )
    await expect(fetchSearchResults({ geoData, units: DEFAULT_UNITS })).rejects.toThrow(/expected/i)
    await expect(fetchSearchResults({ geoData, units: DEFAULT_UNITS })).rejects.toThrow(/received/i)
  })
})
