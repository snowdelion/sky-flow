...
describe("fetchForecastData", () => {
  ...
  it("should throw AppError when cityData is 'not-found'", async () => {
    ...
    await expect(fetchForecastData({ cityData: city, units: DEFAULT_UNITS })).rejects.toThrow(
      /coords not found/i,
    )
  })
  ...
  it("should throw AppError if API returns null", async () => {
    ...
    await expect(fetchForecastData({ cityData, units: DEFAULT_UNITS })).rejects.toThrow(
      /validation failed/i,
    )
    await expect(fetchForecastData({ cityData, units: DEFAULT_UNITS })).rejects.toThrow(/expected/i)
    await expect(fetchForecastData({ cityData, units: DEFAULT_UNITS })).rejects.toThrow(/received/i)
  })

  it("should catch and handle unexpected errors", async () => {
    server.use(http.get("/api/forecast", () => new HttpResponse(null, { status: 500 })))
