...
import { favoriteStore, recentStore, useSearchHistory } from "../useSearchHistory"
...
// --- 2. city factories ---
const createCity = (city: string, country: string, code: string, region: string): CityData => ({
  status: "found",
  city,
  country,
  code,
...
const getTestCities = (): CityData[] =>
  ...
  ].map(({ city, country, code, region }) => createCity(city, country, code, region))
...
describe("useSearchHistory", () => {
  ...
  it("should get recent from localStorage", () => {
    window.localStorage.setItem(STORAGE_KEYS.RECENT, JSON.stringify(warsawHistoryData))
  ...
  it("should add a new city to the recent", () => {
    ...
    const saved = JSON.parse(window.localStorage.getItem(STORAGE_KEYS.RECENT) || "[]")
  ...
  it("should remove from favorite in favorite tab", async () => {
    ...
    await act(() => cities.slice(0, 3).map((cityData) => result.current.addCity(cityData)))
    const tokyoId = result.current.recent[0].id
