...
export const recentStore = new StorageStore(STORAGE_KEYS.RECENT, 8, HistorySchema, [])
export const favoriteStore = new StorageStore(STORAGE_KEYS.FAVORITE, 100, HistorySchema, [])
...
export function useSearchHistory() {
    ...
    recentStore.update((prev) => {
      ...
      if (targetItem) {
        favoriteStore.update((prev) => {
          if (targetItem.isFavorite) return [targetItem, ...prev.filter((item) => item.id !== id)]
  ...
  const removeCity = useCallback(
    (id: string) => recentStore.update((prev) => prev.filter((item) => item.id !== id)),
    [],
  )
