...
export function PageClient({ cityData }: { cityData: CityData }) {
  ...
  if (isError) {
    ...
    if (isAppError && error.code === ERROR_CODES.FORECAST) message = error.message
      ...
      <div className="flex flex-col w-full justify-center items-center">
        <div className="flex flex-col w-full xl:w-auto max-w-99 sm:max-w-full lg:gap-4 xl:gap-8 mb-8 lg:mb-0 items-center lg:items-start lg:flex-row">
          <div className="w-full lg:min-w-150 xl:max-w-200 mb-8">
            <Today currentData={current} forecastUnits={forecastUnits} isPending={isPending} />
            <Details currentData={current} forecastUnits={forecastUnits} isPending={isPending} />
            <DailyForecast dailyData={daily} isPending={isPending} />
          </div>
          <HourlyForecast hourlyData={hourly} forecastUnits={forecastUnits} isPending={isPending} />
        </div>

        <Chart dailyData={daily} hourlyData={hourly} isPending={isPending} devices={devices} />
      </div>
    </>
  )
}
