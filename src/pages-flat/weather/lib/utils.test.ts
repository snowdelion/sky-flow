...
describe("WeatherPage utils", () => {
  ...
  test.each(cases)("should redirect when $name", async ({ params, expected }) => {
  ...
  it("should redirect to default city when no params", async () => {
    ...
    expect(mockRedirect).toHaveBeenCalledWith(expect.stringContaining("/weather/?city=Minsk"))
  })
  ...
  it("should redirect when extra params", async () => {
    ...
    expect(mockRedirect).toHaveBeenCalledTimes(1)
    expect(mockRedirect).toHaveBeenCalledWith(expect.stringContaining("/weather/?city=Berlin"))
    expect(mockRedirect).toHaveBeenCalledWith(expect.not.stringContaining("extra"))
  })
  ...
  it("should redirect when params incorrect", async () => {
    ...
    expect(mockRedirect).toHaveBeenCalledTimes(1)
    expect(mockRedirect).toHaveBeenCalledWith(expect.stringContaining("/weather/?city=Berlin"))
    expect(mockRedirect).toHaveBeenCalledWith(expect.stringContaining("region=State+of+Berlin"))
    expect(mockRedirect).toHaveBeenCalledWith(expect.stringContaining("lat=52.52437"))
    expect(mockRedirect).toHaveBeenCalledWith(expect.stringContaining("lon=13.41053"))
  })
})
